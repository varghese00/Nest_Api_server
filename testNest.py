...
import json
import os
from flask import Flask,Blueprint,jsonify,url_for,render_template,redirect
...
BLYNK_AUTH = 'hr84CGDoMaexTb2nIJsi0ndjXGEHhyM6'
# initialize Blynk
...
##SmartPlug Code
p100 = PyP100.P100("192.168.1.35", "varghese00@gmail.com", "123SmartplugTp") #Creating a P100 plug object
...
while True:
    async def turn_ON_dehumidifier():
        while True:
            blynk.run()
    ...
    async def turn_ON_nest_heating():
        ...
        blynk.virtual_write(4,sense_hat_temp())
        response_json = response.json()
        while True:
            await asyncio.sleep(30)  ## await should be called within the async function for async to work
            ...
            # elif current_sensehat_temperature > heating_ON_temp_threshold:
                # temp_set_to = reset_set_temp
                # uri_setHeat_temperature =  'https://smartdevicemanagement.googleapis.com/v1/' + device_0_name + ':executeCommand'
                # headers = {
                # 'Content-Type': 'application/json',
                # 'Authorization': accesToken,
                ...
                # data = '{"command" : "sdm.devices.commands.ThermostatTemperatureSetpoint.SetHeat", "params" : {"heatCelsius" : ' + str(temp_set_to) + '} }'
                # response = requests.post(uri_setHeat_temperature, headers=headers, data=data)
                # print("Nest Heating is turned off,Temp now is: ", current_sensehat_temperature )
    ...
    loop = asyncio.get_event_loop()   ## event loop must be called for asybc to work

    try:
        asyncio.ensure_future(turn_ON_dehumidifier())
        asyncio.ensure_future(turn_ON_nest_heating())
        loop.run_forever()
...
# nest_readings(thermostat_temperature(),thermostat_humidity())    


# ##For senseHat Temperature and humidity check for automating p100 plug for dedehumidifier and electric heater
...
#     # return turn_ON_nest_heating() decommenting return statement will interept asyncio loop
...
#         high_humidity_threshold = 50
#         await asyncio.sleep(30)
#         if thermostat_humidity() < low_humidity_threshold:
#             print("Turning on de-dehumidifier,ideal humidity is 40-50, nest sensor humidity is: ", thermostat_humidity())
#             p100.turnOn()
#             payload="field1="+str(thermostat_humidity())
#             mqttc.publish(topic, payload)
#             time.sleep(30)
#         elif thermostat_humidity() > low_humidity_threshold:
#             print("Humidity is good downstairs")
...
# # async def turn_ON_dehumidifier():
# #     while True:
# #         low_temp_threshold = 21.7
# #         high_temp_threshold = 21.8
# #         await asyncio.sleep(5)
# #         if sense_hat_temp() < low_temp_threshold:
# #             print("Turning on dehumidifier, sense temp  is: ",sense_hat_temp() )
# #             p100.turnOn()
# #             # return sense_hat_temp
# #         elif sense_hat_temp() > high_temp_threshold:
# #             print("Turning off dehumidifier, sense temp  is: ",sense_hat_temp())
# #             p100.turnOff()
# #             # return sense_hat_temp() // no return if return asyncio wont run the loop or there wont be any exception check
# # loop = asyncio.get_event_loop()   ## event loop must be called for asybc to work
